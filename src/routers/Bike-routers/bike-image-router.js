...
/* banner images related API calls */
// get banner images

bikeImageRouter.get("/banner-images/:count", async (req, res) => {
  try {
    const bannerImages = await BikeDocument.find({ doctype: "banner-image" });
    let count = req.params.count;

    const selectedBannerImages = [];
    ...
    for (let i = 0; i < count; i++) {
      ...
      selectedBannerImages.push(bannerImages[randomIndex]);
    }

    res.send({
      success: {
        data: selectedBannerImages,
        message: `${count} Banner images fetched successfully.`,
      },
    });
  } catch (e) {
    console.log(e);
    res.send({
      error: {
        e,
        message: "something went wrong while fetching banner images",
      },
    });
  }
});
